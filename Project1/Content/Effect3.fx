...
PixelShaderOutput PSObj(VertexShaderOutput input, out float DepthBuffer : DEPTH0)
{
	...
	clip(color.r == 1 && color.g == 0 && color.b == 0 ? -1 : 1); //clip if border
...
output.Color = input.Color.a*Alpha*color*(lerp(AmbientLight, float4(1,1,1,1), l));
	...
	if(HasFog)
		...
		output.Light = float4(l,l,l,1);// light;
...
float4 BorderShader3(float2 uv : TEXCOORD0) : COLOR0
{
	float4 color = tex2D(s, uv);
	float4 current = tex2D(s2, uv);
	float4 top = tex2D(s2, uv - float2(0, 0.005));
	float4 bottom = tex2D(s2, uv + float2(0, 0.005));
	float4 left = tex2D(s2, uv - float2(0.005, 0));
	float4 right = tex2D(s2, uv + float2(0.005, 0));
	/*if (top.r < current.r ||
		bottom.r < current.r ||
		left.r < current.r ||
		right.r < current.r
		)*/
		if (top.r - current.r > 0.005 ||
			bottom.r - current.r  > 0.005 ||
			left.r - current.r  > 0.005 ||
			right.r - current.r > 0.005
		)
...
float4 BorderShader(VertexShaderOutput input, out float DepthBuffer : DEPTH0) : COLOR0
{
	float4 color = tex2D(s, input.TexCoord);
	float4 existingD = tex2D(s2, input.ScreenCoord);
	//clip(input.Depth.x < existingD ? 1 : -1);
	clip(color.a == 0 ? -1 : 1);
	//clip(color == float4(1,0,0,1) ? 1 : -1);

	//if (color.g > 0 && color.b > 0 && color.a > 0)
	//{
	//	DepthBuffer = input.Depth.x;
	...
	if (color.r == 1)
	{
		...
		float yyy = y / TileHeight;//32.0;
		if (
			((input.Color.r == 1) && (input.Color.g == 0) && (input.Color.b == 0) && (input.Color.a == 0) && (yyy < 0.8)) ||
			((input.Color.g == 1) && (input.Color.r == 0) && (input.Color.b == 0) && (input.Color.a == 0) && (xxx < 0.5) && (yyy < 0.8)) ||
			((input.Color.b == 1) && (input.Color.r == 0) && (input.Color.g == 0) && (input.Color.a == 0) && (xxx >= 0.5) && (yyy < 0.8)) ||
			((input.Color.a == 1) && (input.Color.r == 0) && (input.Color.g == 0) && (input.Color.b == 0) && (yyy >= 0.8)))
		{
			//clip(input.Color.r == 1 && yyy < 0.5 ? 1 : -1);
			//clip(input.Color.g == 1 && xxx < 0.5 ? 1 : -1);
			/*clip(input.Color.b == 1 && xxx >= 0.5 ? 1 : -1);
			clip(input.Color.a == 1 && yyy <= 0.5 ? 1 : -1);*/
			DepthBuffer = input.Depth.x;
			return float4(0, 0, 0, 0.5f);// color.a);  //input.Color + float4(0, 0, 0, 1);// float4(0, 0, 0, 1);
		}
	}
	DepthBuffer = input.Depth.x;
	return float4(0, 0, 0, 0);// float4(0, 0, 0, 1);
...
float4 ObjectSelectionShader(VertexShaderOutput input) : COLOR0
{
	float4 color = tex2D(s, input.TexCoord);
	clip(color.a == 0 ? -1 : 1);
	//clip(any(color.gba) ? -1 : 1);
	//clip(color.r < 1 ? -1 : 1);
	clip(color.r < 1 || any(color.gb) ? -1 : 1);
...
technique Technique1
{
    pass Pass1
    {
        ...
        PixelShader = compile ps_4_0 PixelShaderMap();
    }
}
...
technique Technique2
{
    ...
    {
        // TODO: set renderstates here.
		VertexShader = compile vs_4_0 VertexShaderToFile();
        PixelShader = compile ps_4_0 PixelShaderToFile();
    }
}
...
technique RealTime
{
    pass Pass1
    {
        // TODO: set renderstates here.
		VertexShader = compile vs_4_0 VertexShaderFunction();
        PixelShader = compile ps_4_0 PixelShaderRealTime();
    }
}
...
technique TileHighlight
{
    pass Pass1
    {
        // TODO: set renderstates here.
		VertexShader = compile vs_4_0 VertexShaderFunction();
        PixelShader = compile ps_4_0 PixelShaderTileHighlight();
    }
}
...
technique TechniqueObjects
{
    pass Pass1
    {
        ...
        //SrcBlend = SRCALPHA;
		VertexShader = compile vs_4_0 VSObj();//VertexShaderFunction();
        PixelShader = compile ps_4_0 PSObj();//PixelShaderObjects();
    }
}
...
technique TechniqueScenery
{
    pass Pass1
    {
        // TODO: set renderstates here.
		VertexShader = compile vs_4_0 VertexShaderFunction();
        PixelShader = compile ps_4_0 PixelShaderScenery();
    }
}
...
technique BorderShading
{
	pass Pass1
    {
        // TODO: set renderstates here.
		VertexShader = compile vs_4_0 VertexShaderFunction();
        PixelShader = compile ps_4_0 BorderShader3();
    }
}
...
technique Borders
{
    pass Pass1
    {
        // TODO: set renderstates here.
		VertexShader = compile vs_4_0 VertexShaderFunction();
        PixelShader = compile ps_4_0 BorderShader();
    }
}
...
technique Thumbnail
{
	...
	{
		VertexShader = compile vs_4_0 VertexShaderFunction();
        PixelShader = compile ps_4_0 PixelShaderThumbnail();
	}
}
...
technique SmoothLight
{
	...
	{
		VertexShader = compile vs_4_0 VertexShaderFunction();
		PixelShader = compile ps_4_0 PixelShaderSmoothLight();
	}
}

technique ObjectSelection
{
	pass Pass1
	{
		VertexShader = compile vs_4_0 VertexShaderFunction();
		PixelShader = compile ps_4_0 ObjectSelectionShader();
	}
}