...
public sealed class PathingSync
{
    ...
    public void Begin(Actor actor, IntVec3 start, IntVec3 finish, float range = 0)
    {
        var map = actor.Map;
    ...
    private void Fail()
    {
        this.State = Pathing.PathingSync.States.Finished;
    ...
    {
        if (this.State != Pathing.PathingSync.States.Working)
    ...
    static float Heuristic(IntVec3 start, IntVec3 finish)
    {
    ...
    float GetCostTheta(PathNodeBase current, PathNodeBase next)
    {
        ...
        var positionsVec3 = positions.Select(p => new IntVec3(p.X, p.Y, z));
        float cost = 0;
    ...
    float GetCostThetaSingle(IntVec3 current, IntVec3 next)
    {
        var blockCost = Block.GetPathingCost(this.Map, next - IntVec3.UnitZ);
        var stepcost = GetStepCost(current, next);
        var cost = stepcost + blockCost * BlockBaseCost;
        return cost;
    }
    float GetStepCost(IntVec3 a, IntVec3 b)
    {
    ...
    static public bool IsPathable(MapBase map, IntVec3 global)
    {
    ...
    static public bool LineOfSight(MapBase map, IntVec3 a, IntVec3 b, out List<IntVec3> cells)
    {
        if (a.Z == b.Z)
            return LineHelper.LineOfSight(a.X, a.Y, a.Z, b.X, b.Y, b.Z, p => LosPathableFailCondition(map, p), out cells);
        cells = [];// new List<Vector3>();
        return false;
    }
    public static bool LosPathableFailCondition(MapBase map, IntVec3 p)
    {
        var below = p - IntVec3.UnitZ;
...
//{
//    public enum States { Stopped, Working, Finished }
//    public Pathing.PathingSync.States State;
...
//    public Path GetPath()
//    {
//        if (this.State != Pathing.PathingSync.States.Finished)
//            throw new Exception();
//        this.State = Pathing.PathingSync.States.Stopped;
//        if (this.PathInProgress != null)
//        {
...
//        this.State = Pathing.PathingSync.States.Working;
//        this.PathInProgress = new Path();
