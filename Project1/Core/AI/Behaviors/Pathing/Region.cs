...
public sealed class Region : Inspectable
{
    ...
    internal void DrawNode(IntVec3 global, MySpriteBatch sb, Renderer renderer, Camera camera)
    {
        renderer.DrawGridCell(sb, Color.Lime, global);
        var node = this.Nodes[global];//.ToLocal()];
        renderer.DrawGridCells(sb, Color.Lime, node.GetLinks().Select(link => link.Global));
    }
