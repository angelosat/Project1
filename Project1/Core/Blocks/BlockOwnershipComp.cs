...
namespace Project1.Core.Blocks
{
    internal class BlockOwnershipComp : BlockComp
    {
        ...
        {
            yield return new LabelNew(() => $"Owner: {(this.Owner != EntityRefId.Null ? this.Map.World.Get(this.Owner).Name : "<none>")}").InvalidateOn(this.Notifications);
        }
        internal void SetOwner(Actor a)
        {
            var newId = a?.RefId ?? EntityRefId.Null;
