...
public abstract class ToolBlockBuild : ToolManagement, INamed
{
    ...
    {
        var a = new ToolBlockBuild.Args(this.ToolDef, this.Map.ID, start, end, IsRemoving(), IsGodMode(), this.Replacing, orientation);
        this.Callback(a);
        this.Enabled = false;
        this.Replacing = false;
        return a;
    ...
    public class Args
    {
        ...
        public Args(BuildToolDef toolDef, MapId mapid, IntVec3 begin, IntVec3 end, bool modkey, bool cheat, bool replacing = false, int orientation = 0)
        {
            this.MapId = mapid;
