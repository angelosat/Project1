...
struct Shadow
{
    ...
    public readonly void Draw(MySpriteBatch sb, MapView view)
    {
        if (view.IsCompletelyHiddenByFog(this.Global.Z))
            return;
        var zoom = view.Zoom;
        ...
        var pos = view.GetScreenPosition(this.Global).ToFloored();
        Sprite.Shadow.Draw(sb, pos, Color.White * this.Alpha, 0, Sprite.Shadow.OriginGround, zoom * Parent.Body.Scale, SpriteEffects.None, dn);
    }
}
