...
public class ControlTool : IInputEventHandler
{
    ...
    internal virtual void DrawBeforeWorld(MySpriteBatch sb, RenderContext ctx)
    {
    }
    ...
    internal virtual void DrawAfterWorld(MySpriteBatch sb, RenderContext ctx)
    {
        if (this.Target is not null)
            //ToolManager.DrawBlockHighlight(sb, map, camera, this.Target);
            ctx.Renderer.DrawBlockMouseover(sb, this.Target.Global, Color.White);
    }
    ...
    internal virtual void DrawUIRemote(SpriteBatch sb, RenderContext ctx, Vector2 vector2, InteractionTarget targetArgs, PlayerData player)
    {
        UIManager.DrawStringOutlined(sb, this.GetType().Name, vector2 + new Vector2(0, UIManager.Cursor.Height));
    }
    internal virtual void DrawAfterWorldRemote(MySpriteBatch sb, RenderContext ctx, PlayerData player)
    {
    }

    internal virtual void DrawUIRemote(SpriteBatch sb, Renderer camera, PlayerData pl)
    {
    }
