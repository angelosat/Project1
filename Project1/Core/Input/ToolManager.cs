...
public sealed class ToolManager : IInputEventHandler
{
    ...
    static ToolManager()
    {
        ...
        HotkeyManager.RegisterHotkey(HotkeyContextTargeting, "Rotate camera left", () => Ingame.MainViewportCamera.RotateClockwise(), System.Windows.Forms.Keys.Oemcomma);
        HotkeyManager.RegisterHotkey(HotkeyContextTargeting, "Rotate camera right", () => Ingame.MainViewportCamera.RotateCounterClockwise(), System.Windows.Forms.Keys.OemPeriod);
    ...
    internal void DrawBeforeWorld(MySpriteBatch sb, RenderContext ctx)
    {
        if (this.ActiveTool is null)
            return;
        this.ActiveTool.DrawBeforeWorld(sb, ctx);
    }
    internal void DrawAfterWorld(MySpriteBatch sb, RenderContext ctx)
    {
        if (this.ActiveTool is null)
            return;
        this.ActiveTool.DrawAfterWorld(sb, ctx);
        sb.Flush();
    }

    private static void DrawPlayersBlockMouseover(MySpriteBatch sb, RenderContext ctx)
    {
        var map = ctx.Map;
    ...
    internal void DrawUI(SpriteBatch sb, MapView viewport)
    {
        ...
        this.DrawPlayerMousePointers(sb, viewport);
        this.ActiveTool.DrawUI(sb, viewport);
        foreach (var pl in GetOtherPlayers(map))
    ...
    internal void DrawPlayerMousePointers(SpriteBatch sb, MapView viewport)
    {
        ...
        foreach (var pl in GetOtherPlayers(map))
        {
            ...
            {
                Vector2 pos = GetSmoothedMousePosition(viewport, pl);
