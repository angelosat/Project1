using Project1.Core.Helpers;
using Project1.Framework;

namespace Project1.Core.Networking.Simulation;

class PacketChunkReceived
{
    static readonly int _packetTypeId;
    static PacketChunkReceived()
    {
        _packetTypeId = Registry.PacketHandlers.Register(Receive);
    }
    internal static void Init()
    {
    }
    internal static void Send(Client client, PlayerData player, MapId mapid, IntVec2 chunkCoords)
    {
        var w = client.BeginPacket(_packetTypeId);
        w.Write(player.ID);
        w.Write(mapid);
        w.Write(chunkCoords);
    }
    internal static void Receive(NetEndpoint net, Packet p)
    {
        var r = p.PacketReader;
        var playerid = r.ReadInt32();
        var mapid = r.ReadMapId();
        var vec = r.ReadIntVec2();
        GameMode.Current.ChunkReceived(net as Server, playerid, mapid, vec);
    }
}
