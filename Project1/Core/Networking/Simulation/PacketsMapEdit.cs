...
internal static class PacketsMapEdit
{
    ...
    private static void OnMapEdit(NetEndpoint endpoint, Packet packet)
    {
        ...
        if (type == MapEditType.Create)
            MapEdit.Paint(context, map, targets, blockdef.Block, materialdef, data, variation, orientation);
        else if (type == MapEditType.Replace)
            MapEdit.PaintWithOrigin(context, map, targets, blockdef.Block, materialdef, data, variation, orientation);
    }
}
