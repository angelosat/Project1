...
public sealed class ResourcesComp : EntityComp
{
    ...
    readonly Dictionary<ResourceDef, ResourceRuntime> Resources = [];
    ...
    {
        var res = new ResourceRuntime(def).Bind(this.Owner);
    ...
    public override void Tick()
    {
        ...
        {
            item.Tick();
            item.Accumulator.Tick(out var delta);
            if (delta != 0)
                this.ApplyDelta(item.Def, delta);
        }
    }
    ...
    internal void ApplyDelta(ResourceDef def, int delta)
    {
        var res = this.Resources[def];
        res.ApplyDelta(delta);
        this.Owner.World?.Events.Post(new ResourceDeltaAppliedEvent(this.Owner, def, delta));
    }
    internal void ApplyAccumulatorRateDelta(ResourceDef def, float delta)
    {
        var res = this.Resources[def];
        res.ApplyAccumulatorRateDelta(delta);
    }
    internal void ApplyAccumulatorDelta(ResourceDef def, float delta)
    {
        var res = this.Resources[def];
        res.ApplyAccumulatorDelta(delta);
    }
    public int GetMax(ResourceDef def)
        => this.Resources[def].Max;
    public void SetMax(ResourceDef def, int max)
    {
    ...
    public void SetValue(ResourceDef def, int value)
    {
        var res = this.Resources[def];
        res.SetValue(value);
        this.Owner.World?.Events.Post(new ResourceChangedEvent(this.Owner, res));
    ...
    internal ResourceRuntime GetResource(ResourceDef def)
       => this.Resources[def];
    ...
    internal int GetValue(ResourceDef def)
        => this.Resources[def].Value;
    internal int GetDeficit(ResourceDef def)
    ...
    public record class EntityResourceView(ResourcesComp Comp, ResourceDef Def) : IResourceView
    {
        ...
        public void ApplyDelta(int delta) => this.Comp.ApplyDelta(this.Def, delta);

        public void ApplyAccumulatorRateDelta(float delta) => this.Comp.ApplyAccumulatorRateDelta(this.Def, delta);
        public void ApplyAccumulatorDelta(float delta) => this.Comp.ApplyAccumulatorDelta(this.Def, delta);
    ...
    [Obsolete]
    public record class EntityResourceViewOld(ResourcesComp Comp, ResourceRuntime Resource) : IResourceView
    {
