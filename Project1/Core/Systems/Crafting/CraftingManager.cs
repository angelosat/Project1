...
public sealed class CraftingManager : TownComp
{
    ...
    {
        if(this.commitmentsByActor.TryGetValue(e.Actor, out var commitment))
        {
            // what if the plan is for crafting this order??
            //this.Uncommit(e.Actor);
        }
    ...
    {
        if (this.commitmentsByIngredients.ContainsKey(item.RefId))
            return true;
        if (this.UnfinishedItems.Contains(item)) // additionally check if the unfinished item's order is currently worked on?
            return true;
        return false;
    }
