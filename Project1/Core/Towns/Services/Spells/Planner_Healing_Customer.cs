...
internal class Planner_Healing_Customer : Planner
{
    protected override Plan TryPlan(Actor actor)
    {
        ...
        if (manager.TryGetRequestByTarget(actor, out var existing))
            return ResolveRequest(actor, existing);

        if (actor.IsHauling)
            return null;

        var cash = actor.Inventory.Count(ItemDefOf.Coins);
        ...
        var (tag, score) = scored.FirstOrDefault(a => a.tag.Price <= cash);
    ...
    private static Plan ResolveRequest(Actor actor, ServiceRequest_Spell existing)
    {
        if (existing.Vendor == EntityRefId.Null)
            return null;
        ...
        if (existing.IsPaidFor)
        {
            if (existing.IsCasterReady && !actor.IsHauling)
                return new Plan(TownServiceSpellsDefOf.PlanHealingSeek, caster) { ServiceRequest = existing.Id };
            return null;
        }
        else
        {
            ...
            if (actor.Hauled is Entity carried)
            {
                ...
                return new Plan(PlanDefOf.TradeOffer, caster) { ServiceRequest = existing.Id, TradeId = trade.Id };
            }
            else
            {
                var money = actor.Inventory.First(i => i.Def == ItemDefOf.Coins);
                return new Plan(PlanDefOf.RetrieveFromInventory, money) { ServiceRequest = existing.Id, AmountA = existing.Price };
            }
        }
