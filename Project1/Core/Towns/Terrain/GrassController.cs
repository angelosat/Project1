...
internal sealed class GrassController(Chunk chunk) : ChunkController(chunk)
{
    ...
    private void HandleEntityEnteringNewCell(EntityEnteringNewCellEvent e)
    {
        if (e.Entity.Net.IsClient)
            return;
        var cell = e.Entity.Cell.Below;
    ...
    {
        this.Map.Events.ListenTo<EntityEnteringNewCellEvent>(HandleEntityEnteringNewCell);
        return;
        Reset();
    }
