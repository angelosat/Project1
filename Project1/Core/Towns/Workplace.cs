...
public abstract class Workplace : ISerializable, ISaveable
{
    ...
    class Gui : GroupBox
    {
        ...
        public Gui()
        {
            ...
            this.LabelName = new Label(() => this.SelectedShop?.Name ?? "", () => { DialogInput.ShowInputDialog("Enter new name", nn => Packets.SendPlayerRenameShop (this.SelectedShop.Net, this.SelectedShop.Net.GetPlayer().ID, SelectedShop.Map.ID, this.SelectedShop.ID, nn), 16, this.SelectedShop.Name); });
            this.ChkBoxEnabled = new("Open", () => this.SelectedShop?.Active ?? false, () => Packets.SendPlayerToggleShop(this.SelectedShop.Net, this.SelectedShop.Net.GetPlayer().ID, SelectedShop.Map.ID, this.SelectedShop.ID));
            int listw = 200, listh = 300;
        ...
        {
            var net = st.Net;
            PacketsWorkplaces.SendPlayerAddStockpileToShop(net, net.GetPlayer().ID, this.SelectedShop.Map, this.SelectedShop.ID, st.ID);
        }
        void TickShoppingArea(Stockpile st)
        {
            var net = st.Net;
            PacketsWorkplaces.SendPlayerAddShoppingArea(net, net.GetPlayer().ID, st.Map.ID, this.SelectedShop.ID, st.ID);
        }
    }
    ...
    class Packets
    {
        ...
        static void UpdateWorkerRoles(NetEndpoint net, Packet pck)
        {
            ...
            var tavern = map.Town.GetShop(r.ReadInt32());
        ...
        static public void SendPlayerRenameShop(NetEndpoint net, int playerID, MapId mapId, int shopID, string name)
        {
            if (shopID < 0)
                return;
        ...
        private static void ReceivePlayerRenameShop(NetEndpoint net, Packet pck)
        {
            ...
            if (net is Server)
                SendPlayerRenameShop(net, playerID, mapid, shopid, name);
        }

        static public void SendPlayerToggleShop(NetEndpoint net, int playerID, MapId mapId, int shopID)
        {
            if (shopID < 0)
                return;
        ...
        private static void ReceivePlayerToggleShop(NetEndpoint net, Packet pck)
        {
            ...
            if (net is Server)
                SendPlayerToggleShop(net, playerID, mapid, shopid);
        }
    }
