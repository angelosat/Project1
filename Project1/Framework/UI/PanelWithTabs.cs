...
{
    public static readonly int DefaultSize = 256, DefaultTabsListSize = 64;// 96; //128
    Control SelectedTab;
...
public class PanelWithVerticalTabs<T> : GroupBox where T : ButtonBase, new()
{
    ...
    public PanelWithVerticalTabs(int w, int h)
    {
        ...
        this.TabsList = new(c => new T() { Text = c.Name, Active = true, LeftClickAction = () => SelectTab(c), 
            Width = DefaultTabsListSize,//128, 
            IsToggledFunc = () => this.SelectedTab == c });
        Panel panel = null, panelTabs;
        //ScrollableBoxNewNewNew
        //    panelClientTabs = null;
        //panelClientTabs = new ScrollableBoxNewNewNew(128 + ScrollbarV.DefaultWidth, h, ScrollModes.Vertical);
        var panelClientTabs = ScrollableBoxNewNewNew.FromWidth(this.TabsList, DefaultTabsListSize, h);
        //panelClientTabs.AddControls(this.TabsList);
        this.PanelClient = new ScrollableBoxNewNewNew(w + ScrollbarV.DefaultWidth, h, ScrollModes.Vertical) { Autoscroll = true };
        panel = new() { AutoSize = true };
        panelTabs = new() { AutoSize = true };
        panel.AddControls(this.PanelClient);
        panelTabs.AddControls(panelClientTabs);
