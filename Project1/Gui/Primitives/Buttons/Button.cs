...
namespace Start_a_Town_.UI
{
    public class ButtonNewNew : ButtonBaseNew
    {
        ...
        public ButtonNewNew(int width) : this("", width)
        {
            this.Width = width;
            if (width > 0)
                this.AutoSize = false;
        }
        public ButtonNewNew(SpriteFont font, string text, int width = 0)
            : this()
        {
            this.Font = font;
            Text = text;
        ...
        public ButtonNewNew(Func<string> text, Action action, int width = 0) : this(width)
        {
            this.TextFunc = text;
            this.LeftClickAction = action;
        }
        public ButtonNewNew(string text, int width = 0)
            : this()
        {
            Text = text;
            if (width > 0)
        ...
        {
            ButtonNewNew.Draw(sb, this, Vector2.Zero, 1);// (MouseHover || IsPressed) ? 1 : 0.5f);
        }

        static public void Draw(SpriteBatch sb, ButtonNewNew button, Vector2 location, float opacity = 1)
        {
            ...
            var actualwidth = button.Width - ButtonNewNew.SpriteLeft.Width - ButtonNewNew.SpriteRight.Width;

            var pos = new Vector2(ButtonNewNew.SpriteLeft.Width + actualwidth * halign, ButtonNewNew.SpriteCenter.Height / 2 + (button.IsPushed ? 1 : 0));
        ...
        static public void DrawSprite(SpriteBatch sb, ButtonNewNew button, Rectangle destRect, Color color, float opacity, SpriteEffects sprFx)
        {
            ...
            sb.Draw(UIManager.DefaultButtonSprite,
                new Rectangle(
                    destRect.X + ButtonNewNew.SpriteLeft.Width,
                    destRect.Y,
                    destRect.Width - ButtonNewNew.SpriteLeft.Width - ButtonNewNew.SpriteRight.Width,
                    SpriteCenter.Height),
                ButtonNewNew.SpriteCenter, c, 0, Vector2.Zero, fx, 0);
            sb.Draw(UIManager.DefaultButtonSprite, new Vector2(destRect.X + destRect.Width - ButtonNewNew.SpriteRight.Width, destRect.Y), ButtonNewNew.SpriteRight, c, 0, Vector2.Zero, 1, fx, 0);
        }
